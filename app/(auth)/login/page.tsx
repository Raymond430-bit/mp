...
import { Zap, Mail, Lock, Eye, EyeOff, Shield, AlertTriangle, Loader2 } from "lucide-react";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/auth/login", {
        ...
        body: JSON.stringify({ email, password, rememberMe, csrfToken: "placeholder" }),
      });

      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Login failed");
      if (data.requires2FA) { router.push("/verify-2fa"); return; }
    ...
    <div className="min-h-screen flex items-center justify-center px-4 relative">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-gray-400">Sign in to your secure trading account</p>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required
                className="input-field pl-12" placeholder="you@example.com" autoComplete="email" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)}
                required minLength={12} className="input-field pl-12 pr-12" placeholder="••••••••••••" autoComplete="current-password" />
          ...
          <div className="flex items-center justify-between">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)}
                className="w-4 h-4 rounded border-mp-border bg-mp-surface text-mp-cyan focus:ring-mp-cyan/30" />
              <span className="text-sm text-gray-400">Remember me for 30 days</span>
            </label>
            <Link href="/forgot-password" className="text-sm text-mp-cyan hover:text-mp-purple transition-colors">Forgot password?</Link>
          </div>

          <button type="submit" disabled={loading} className="btn-primary w-full flex items-center justify-center gap-2 disabled:opacity-50">
            {loading ? <><Loader2 className="w-5 h-5 animate-spin" /> Signing in...</> : <>Sign In <Zap className="w-5 h-5" /></>}
          </button>

          <div className="relative">
            <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-mp-border" /></div>
            <div className="relative flex justify-center text-sm"><span className="px-4 bg-mp-surface text-gray-500">or continue with</span></div>
          </div>
        ...
        <p className="mt-6 text-center text-sm text-gray-400">
          Don&apos;t have an account? <Link href="/register" className="text-mp-cyan hover:text-mp-purple transition-colors font-medium">Create one now</Link>
        </p>
      </motion.div>
    </div>
  );
